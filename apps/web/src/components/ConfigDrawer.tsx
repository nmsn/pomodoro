    ...
    <Drawer direction="right">
      <DrawerTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className="h-10 w-10 rounded-lg shadow-md bg-background/80 backdrop-blur-sm border-muted-foreground/20 hover:bg-background/90"
        >
...
function Sidebar({ navItems, selectedNav, onNavSelect }: SidebarProps) {
  const renderNavItem = (item: NavItem) => {
    ...
    return (
      <button
        ...
        className={cn(
          "group w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 cursor-pointer relative overflow-hidden",
          isSelected
  ...
  return (
    <div className="w-52 border-r bg-muted/20 flex-shrink-0 flex flex-col">
      {/* 关闭按钮 */}
